(function (exports, $) {
	...
	exports.TeambuilderRoom = exports.Room.extend({
		...
		renderSet: function (set, i) {
			...
			var isNatDex = this.curTeam.format.includes('nationaldex') || this.curTeam.format.includes('natdex') || this.curTeam.format.includes('modded') || this.curTeam.format.includes('poa') || this.curTeam.format.includes('altermons');
			var isDenise = this.curTeam.format.includes('denise');
			var buf = '<li value="' + i + '">';
			...
			if (this.curTeam.gen > 1) {
				...
				} else if (this.curTeam.gen < 8 || isNatDex || isDenise) {
				...
				if (!isLetsGo && this.curTeam.gen < 9) {
					if (this.curTeam.gen === 8 && !isNatDex || !isDenise) {
				...
				if (this.curTeam.gen === 9 && !isDenise) {
			...
			var table = null;
			if (this.curTeam.mod === 'gen3denise') {
				table = BattleTeambuilderTable[this.curTeam.mod];
			} else if (this.curTeam.gen < 7) {
				table = BattleTeambuilderTable['gen' + this.curTeam.gen];
			}
			if (
		...
		updateDetailsForm: function () {
			...
			var isLetsGo = this.curTeam.format.includes('letsgo');
			var isBDSP = this.curTeam.format.includes('bdsp');
			var isNatDex = this.curTeam.format.includes('nationaldex') || this.curTeam.format.includes('natdex') || this.curTeam.format.includes('poa') || this.curTeam.format.includes('altermons');
			var isDenise = this.curTeam.format.includes('denise');
			...
			if (this.curTeam.gen > 1) {
				...
				if (isLetsGo) {
					...
					if (this.curTeam.gen < 8 || isNatDex || isDenise) buf += '<div class="formrow"><label class="formlabel">Happiness:</label><div><input type="number" min="0" max="255" step="1" name="happiness" value="' + (typeof set.happiness === 'number' ? set.happiness : 255) + '" class="textbox inputform numform" /></div></div>';
				}
			...
			if (!isLetsGo && (this.curTeam.gen === 7 || isNatDex || (isBDSP && species.baseSpecies === 'Unown')) || isDenise) {
		...
		detailsChange: function (e) {
			...
			var isBDSP = this.curTeam.format.includes('bdsp');
			var isNatDex = this.curTeam.format.includes('nationaldex') || this.curTeam.format.includes('natdex') || this.curTeam.format.includes('poa') || this.curTeam.format.includes('altermons');
			var isDenise = this.curTeam.format.includes('denise') || this.curTeam.format.includes('fakemons');
			...
			if (this.curTeam.gen > 1) {
				...
				if (isLetsGo) {
					...
					if (this.curTeam.gen < 8 || isNatDex || isDenise) buf += '<span class="detailcell"><label>Happiness</label>' + (typeof set.happiness === 'number' ? set.happiness : 255) + '</span>';
				}
				buf += '<span class="detailcell"><label>Shiny</label>' + (set.shiny ? 'Yes' : 'No') + '</span>';
				if (!isLetsGo && (this.curTeam.gen < 8 || isNatDex || isDenise)) buf += '<span class="detailcell"><label>HP Type</label>' + (set.hpType || 'Dark') + '</span>';
				if (this.curTeam.gen === 8 && !isBDSP) {
					if (!species.cannotDynamax) {
						buf += '<span class="detailcell"><label>Dmax Level</label>' + (typeof set.dynamaxLevel === 'number' ? set.dynamaxLevel : 10) + '</span>';
					}
