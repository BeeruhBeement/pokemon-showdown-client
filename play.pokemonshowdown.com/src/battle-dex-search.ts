...
abstract class BattleTypedSearch<T extends SearchType> {
   ...
   modFormat = '' as ID;
	...
	constructor(searchType: T, format = '' as ID, speciesOrSet: ID | PokemonSet = '' as ID) {
		...
		if (format.includes('nationaldex') || format.startsWith('nd') || format.includes('natdex') || format.includes('modded') || format.includes('poa') || format.includes('altermons') || format.includes('hellskitchen') || format.includes('buildmons')) {
