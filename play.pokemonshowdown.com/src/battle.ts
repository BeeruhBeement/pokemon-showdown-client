...
export class Side {
	...
	addSideCondition(effect: Effect, persist: boolean) {
		...
		case 'mist':
			...
			break;
		case 'tailwind':
			this.sideConditions[condition] = [effect.name, 1, (this.battle.gen >= 5 || this.battle.mod === 'gen3denise') ? persist ? 6 : 4 : persist ? 5 : 3, 0];
			break;
		case 'luckychant':
			this.sideConditions[condition] = [effect.name, 1, 5, 0];
			break;
	...
	changeWeather(weatherName: string, poke?: Pokemon, isUpkeep?: boolean, ability?: Effect) {
		...
		if (weather) {
			...
			if (poke) {
				...
				this.weatherTimeLeft = ((this.gen <= 5 && !this.tier.includes("Denise")) || isExtremeWeather) ? 0 : 8;
				this.weatherMinTimeLeft = ((this.gen <= 5 && !this.tier.includes("Denise")) || isExtremeWeather) ? 0 : 5;
			} else if (isExtremeWeather) {
				this.weatherTimeLeft = 0;
				this.weatherMinTimeLeft = 0;
			} else if (this.tier.includes("Denise")) {
